...
def obtener_suscripcion_a_topico():
    json_schema = consultar_schema_registry('public/default/eventos-processed_image')  
    avro_schema = obtener_schema_avro_de_diccionario(json_schema)

    client = pulsar.Client(f'pulsar://{broker_host()}:6650')
    return client.subscribe('eventos-processed_image', consumer_type=_pulsar.ConsumerType.Shared, subscription_name='saludtech-bff', schema=avro_schema)