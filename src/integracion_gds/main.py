...
from .eventos import EventoConfirmacionGDS, ConfirmacionRevertida, ProcessedImageConfirmada
from .comandos import ComandoConfirmarProcessedImage, ComandoRevertirConfirmacion, ConfirmarProcessedImagePayload, RevertirConfirmacionPayload
...
settings = Config()
app_configs: dict[str, Any] = {"title": "Pagos saludtech"}
...
async def app_startup():
    ...
    task2 = asyncio.ensure_future(suscribirse_a_topico("comando-confirmar-processed_image", "sub-com-gds-confirmacion", ComandoConfirmarProcessedImage))
...
@app.get("/prueba-processed_image-confirmada", include_in_schema=False)
async def prueba_processed_image_confirmada() -> dict[str, str]:
    payload = ProcessedImageConfirmada(
        id = "1232321321",
        id_correlacion = "389822434",
        processed_image_id = "6463454",
    ...
    evento = EventoConfirmacionGDS(
        ...
        datacontenttype=ProcessedImageConfirmada.__name__,
        processed_image_pagada = payload
    )
...
@app.get("/prueba-confirmar-processed_image", include_in_schema=False)
async def prueba_confirmar_processed_image() -> dict[str, str]:
    payload = ConfirmarProcessedImagePayload(
        id_correlacion = "389822434",
        processed_image_id = "6463454",
    )

    comando = ComandoConfirmarProcessedImage(
        time=utils.time_millis(),
        ingestion=utils.time_millis(),
        datacontenttype=ConfirmarProcessedImagePayload.__name__,
        data = payload
    )
    despachador = Despachador()
    despachador.publicar_mensaje(comando, "comando-confirmar-processed_image")
    return {"status": "ok"}
