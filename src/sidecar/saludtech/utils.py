...
"""def dict_a_proto_locacion(dict_locacion):
    return processed_datas_pb2.Locacion(codigo=dict_locacion.get('codigo'), nombre=dict_locacion.get('nombre'))
        ...
        for odo in itin.get('odos', []):
            ...
            for seg in odo.get('segmentos', []):
                ...
                for leg in seg.get('legs', []):
                    ...
                    legs.append(processed_datas_pb2.Leg(fecha_llegada=fecha_llegada, fecha_salida=fecha_salida, origen=origen, destino=destino))
                
                segmentos.append(processed_datas_pb2.Segmento(legs=legs))
            odos.append(processed_datas_pb2.Odo(segmentos=segmentos))
        itinerarios.append(processed_datas_pb2.Itinerario(odos=odos))
    return itinerarios
"""