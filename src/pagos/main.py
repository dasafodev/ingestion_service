...
from .eventos import EventoPago, PagoRevertido, ProcessedImagePagada
from .comandos import ComandoPagarProcessedImage, ComandoRevertirPago, RevertirPagoPayload, PagarProcessedImagePayload
...
async def app_startup():
    ...
    task2 = asyncio.ensure_future(suscribirse_a_topico("comando-pagar-processed_image", "sub-com-pagos-processed_imager", ComandoPagarProcessedImage))
...
@app.get("/prueba-processed_image-pagada", include_in_schema=False)
async def prueba_processed_image_pagada() -> dict[str, str]:
    payload = ProcessedImagePagada(
        id = "1232321321",
    ...
    evento = EventoPago(
        ...
        datacontenttype=ProcessedImagePagada.__name__,
...
@app.get("/prueba-pagar-processed_image", include_in_schema=False)
async def prueba_pagar_processed_image() -> dict[str, str]:
    payload = PagarProcessedImagePayload(
        id_correlacion = "389822434",
        processed_image_id = "6463454",
        monto = 23412.12,
        monto_vat = 234.0,
    ...
    comando = ComandoPagarProcessedImage(
        time=utils.time_millis(),
        ingestion=utils.time_millis(),
        datacontenttype=ProcessedImagePagada.__name__,
        data = payload
    ...
    despachador = Despachador()
    despachador.publicar_mensaje(comando, "comando-pagar-processed_image")
    return {"status": "ok"}
